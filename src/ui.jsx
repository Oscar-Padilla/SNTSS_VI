...
export function QR({ seed = "SNTSS", size }) {
  const N = 11;
  let h = 0;
      ...
      {open && (
        <div style={{ borderTop: "1px solid var(--line)", background: "#fff" }}>
          <div className="container" style={{ padding: "12px 24px 18px" }}>
            ...
            <button className="btn btn-outline btn-block" style={{ marginTop: 10 }} onClick={() => nav("acceso")}>
...
export function Pill({ type = "purple", children, icon }) {
  return (
    <span className={"pill pill-" + type}>
      {icon ? <Icon name={icon} size={13} /> : <i className="dot" />}
      {children}
    </span>
  );
}

export function SecHead({ eyebrow, title, sub }) {
  return (
...
export function ProgressEvent({ inscritos, cupo }) {
