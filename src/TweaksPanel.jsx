...
const STYLE = `
  ...
  .twk-float-btn{position:fixed;right:16px;bottom:16px;z-index:2147483645;
    width:40px;height:40px;border-radius:50%;background:rgba(41,38,27,.78);
    color:#fff;border:none;font-size:17px;cursor:pointer;display:flex;
    align-items:center;justify-content:center;
    box-shadow:0 2px 10px rgba(0,0,0,.28);transition:background .15s}
  .twk-float-btn:hover{background:rgba(41,38,27,.92)}
`;
...
export function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  if (!open) {
    return (
      <>
        <style>{STYLE}</style>
        <button className="twk-float-btn" onClick={() => setOpen(true)} title="Tweaks">⚙</button>
      </>
    );
  }
