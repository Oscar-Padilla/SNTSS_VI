...
import { Brand, Pill, QR } from './ui.jsx'
import DATA from './data.js'

const D = DATA;

export function PageAcceso({ go, toast }) {
      ...
      <div className="auth-form">
        <div className="inner">
          ...
          <form onSubmit={(e) => { e.preventDefault(); role === "admin" ? go("admin") : go("registro"); }}>
            <div className="field">
              <label>{role === "admin" ? "Usuario de secretaría" : "Matrícula"}</label>
              <input className="input" defaultValue={role === "admin" ? "igualdad.sustantiva" : ""} placeholder={role === "admin" ? "" : "Tu matrícula IMSS"} />
            </div>
  ...
  return (
    <div className="admin">
      <aside className="admin-side">
        ...
        {nav.map(([id, l, ic]) => (
          <a key={id} href="#"
             className={"admin-nav" + (tab === id || (tab === "crear" && id === "convocatorias") ? " active" : "")}
      ...
      <main className="admin-main">
        ...
        {tab === "resumen" && <AdminResumen setTab={setTab} />}
...
function LegendRow({ c, l, v }) {
  return (
    <div className="row items-center justify-between">
      <span className="row items-center gap-8">
        <i style={{ width: 12, height: 12, borderRadius: 4, background: c }} />
        <span className="small" style={{ fontWeight: 600 }}>{l}</span>
      </span>
      <b style={{ fontFamily: "var(--head)" }}>{v}</b>
    </div>
  );
}
          ...
          <table className="data">
            ...
            <tbody>
              {D.eventos.map((e) => (
                <tr key={e.id}>
                  <td>
                    <b style={{ fontFamily: "var(--head)" }}>{e.titulo}</b><br />
                    <span className="small" style={{ color: "var(--ink-faint)" }}>{e.secretaria}</span>
                  </td>
                  <td>{e.fecha}</td>
                  <td>{e.cupo}</td>
        ...
        <form onSubmit={(e) => { e.preventDefault(); toast && toast("Convocatoria publicada"); setTab("convocatorias"); }}>
          ...
          <div className="field">
            ...
            <div className={"upload" + (foto ? " done" : "")}>
              ...
              <div className="txt">
                <b>{foto ? "imagen-evento.jpg" : "Sube una foto de portada"}</b>
                <p>{foto ? "Cargada correctamente" : "JPG o PNG · máx 3 MB"}</p>
              </div>
      ...
      <div className="stack gap-16">
        ...
        <div className="card" style={{ overflow: "hidden" }}>
          ...
          <div className="card-pad">
            ...
            <div className="row items-center gap-8 small" style={{ color: "var(--ink-faint)", fontWeight: 600 }}>
              <Icon name="users" size={14} /> {reqs.length} documentos requeridos
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card card-pad">
        <div className="row items-center justify-between wrap gap-12" style={{ marginBottom: 16 }}>
          <div className="row items-center gap-8" style={{ flex: 1, minWidth: 220, maxWidth: 360, border: "1.5px solid var(--line-strong)", borderRadius: 10, padding: "0 12px" }}>
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar por nombre o folio…"
              style={{ border: "none", outline: "none", padding: "11px 0", width: "100%", fontFamily: "var(--body)", fontSize: ".95rem", background: "transparent" }} />
          </div>
  ...
  return (
    <div className="table-wrap">
      <table className="data">
        <thead>
          <tr>
            <th>Folio</th><th>Nombre</th><th>Unidad</th>
            {!compact && <th>Documentos</th>}
            <th>Estado</th><th>Asistencia</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => (
