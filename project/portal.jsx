/* Portal público. Exporta páginas a window. */
const D = window.DATA;
...
/* Grid de noticias reutilizable */
function NewsGrid({ go, limit }) {
  const items = D.noticias.slice(0, limit || D.noticias.length);
  const [feat, ...rest] = items;
  return (
    <div className="grid news-grid">
      <div className="card news-card feat card-hover" onClick={() => go("nota", { id: feat.id })} style={{ cursor: "pointer", gridRow: "span 1" }}>
...
Object.assign(window, { PageInicio, EventCard, NewsGrid });
