/* Portal público — páginas secundarias */
const { useState } = React;
const D = window.DATA;

function PageHero({ crumb, title, sub, go }) {
  return (
        ...
        <div className="container">
          ...
          <div className="grid feature-grid">
            {[["users", "Cercanía", "Atendemos a cada trabajador por su nombre, no por un número de folio."],
              ...
              <div className="feature" key={t}><div className="ico ico-lg"><Icon name={i} size={26} /></div><h3 className="h3">{t}</h3><p>{d}</p></div>
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="container">
          <div className="tint-box row items-center justify-between wrap gap-16">
            <div className="row items-center gap-16">
              <span className="ico ico-lg" style={{ width: 50, height: 50, borderRadius: 14, display: "grid", placeItems: "center", background: "#fff", color: "var(--purple)" }}><Icon name="phone" size={24} /></span>
              <div><b className="h3">¿Eres delegado de tu unidad?</b><p className="small">Solicita acceso para administrar las convocatorias de tu área.</p></div>
            </div>
      ...
      <section className="section">
        <div className="container" style={{ maxWidth: 800 }}>
          <div className="row items-center gap-12" style={{ marginBottom: 22 }}>
            ...
            <span className="small" style={{ color: "var(--ink-faint)", fontWeight: 600 }}><Icon name="clock" size={14} style={{ verticalAlign: "-2px" }} /> {n.fecha} · {n.autor}</span>
          </div>
...
/* ===================== CONTACTO ===================== */
function PageContacto({ go, toast }) {
              ...
              {sent ? (
                ...
                <form onSubmit={(e) => { e.preventDefault(); setSent(true); toast && toast("Mensaje enviado correctamente"); }}>
                  ...
                  <div className="field"><label>Asunto</label>
                    <select className="select"><option>Información de un evento</option><option>Problema con mi credencial</option><option>Becas</option><option>Otro</option></select>
                  </div>
            ...
            <div className="stack gap-16">
              {[["pin", "Oficinas de la Sección VI", "Calle 60 x 65, Centro, Mérida, Yucatán"],
                ...
                <div className="card card-pad row items-center gap-16" key={t}>
                  <span className="ico ico-lg" style={{ width: 50, height: 50, borderRadius: 14, display: "grid", placeItems: "center", background: "var(--purple-50)", color: "var(--purple)", flexShrink: 0 }}><Icon name={i} size={24} /></span>
              ...
              <div className="card" style={{ overflow: "hidden", background: "var(--canvas-2)" }}>
                <div style={{ aspectRatio: "16/9", display: "grid", placeItems: "center", color: "var(--ink-faint)", background: "repeating-linear-gradient(45deg, var(--canvas-2), var(--canvas-2) 12px, #fff 12px, #fff 24px)" }}>
                  <div className="center"><Icon name="pin" size={32} style={{ color: "var(--purple)" }} /><p className="small" style={{ marginTop: 8, fontWeight: 700 }}>Mapa de ubicación</p></div>
                </div>
              </div>
            </div>
          </div>
...
Object.assign(window, { PageHero, PageNosotros, PageDelegados, PageEventos, PageNoticias, PageNota, PageContacto });
