/* Flujo del trabajador: registro → inscripción → credencial QR */
const { useState, useEffect } = React;
const D = window.DATA;

function MemberFlow({ go, params, toast }) {
  const W = D.worker;
  const evento = D.eventos.find((e) => e.id === (params && params.evento)) || D.eventos[0];
  const [step, setStep] = useState(0);
  const [docs, setDocs] = useState({
    credencial: true,   // ya guardado
    afiliacion: true,   // ya guardado
    acta: false,
    curp: false,
  });
  ...
  return (
    <div>
      <PageHero go={go} crumb="Mi credencial" title={step === 3 ? "¡Inscripción confirmada!" : "Inscripción al evento"}
            ...
            <div className="card card-pad">
              ...
              <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                {[["Matrícula", W.matricula], ["Categoría", W.categoria], ["Adscripción", W.adscripcion], ["Antigüedad", W.antiguedad]].map(([k, v]) => (
                  <div className="field" key={k} style={{ marginBottom: 0 }}><label>{k}</label><input className="input" defaultValue={v} readOnly style={{ background: "var(--canvas)" }} /></div>
                ))}
              </div>
          ...
          {step === 3 && (
            <div className="stack gap-24 items-center">
              <div className="card card-pad center" style={{ maxWidth: 560 }}>
                ...
                <p style={{ color: "var(--ink-soft)" }}><Icon name="calendar" size={15} style={{ verticalAlign: "-2px" }} /> {evento.fecha} · {evento.hora} · {evento.lugar}</p>
...
/* Fila de documento */
function DocRow({ label, fmt, saved, done, onUpload }) {
  if (saved) {
    return (
...
Object.assign(window, { MemberFlow, DocRow, CredentialCard });
